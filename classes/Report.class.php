<?php
    class Report{
        ...
        public static function textReport($userId, $postId)
        {
            $conn = Db::getInstance();
            $txtStmnt = $conn->prepare('select * from reports where post_id = :postId and user_id = :userId and statusReport = :status');
            $txtStmnt->bindValue(':postId', $postId);
            $txtStmnt->bindValue(':userId', $userId);
            $txtStmnt->bindValue(':status', 1);
            $txtStmnt->execute();
            if($txtStmnt->rowCount() === 1){
                return true;
            }
            else{
                return false;
